...
```bash
claude mcp add latam-data -- /path/to/.venv/bin/python /path/to/server.py
```

Remote (HTTP) transport:

```bash
.venv/bin/python -c "from server import mcp; mcp.run(transport='http', host='0.0.0.0', port=8000)"
```
